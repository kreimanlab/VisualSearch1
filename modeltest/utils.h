...
#define NBS3PROTS 600 // Number of S3 prototypes / patches / templates
#define MAXPROTS 600
...
#define MAXLAYERS 50000
#define MAXNEUR 100000000
...
void loadpic(const char *filename)
{
	...
	unsigned char realpic[IMAGEXSIZE*IMAGEYSIZE];
	float max = -9999;
	for (int n=0; n < MAXNEUR; n++)
		v[n] = 0;
	if (!strstr(filename, "raw")) mydie("Image files must be in the raw format (see prepimages.m, utils.h)!\n");
	FILE *f = fopen(filename, "r");
	if (!f) { printf("Couldn't open the image file [%s]!", filename); mydie("Bye!"); }
	//printf("Reading image %s\n", filename);
	for (int i=0; i < IMAGEXSIZE * IMAGEYSIZE; i++)
		if( !fread(&realpic[i], 1, 1, f)) mydie("Image file does not contain enough pixel values!\n!");
	fclose(f);
	for (int i=0; i < IMAGEXSIZE*IMAGEYSIZE; i++)
	{
